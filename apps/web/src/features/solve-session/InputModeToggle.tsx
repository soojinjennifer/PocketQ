...
const SEGMENTS: readonly InputMode[] = ["photo", "handwriting"];
...
const SEGMENT_BASE_STYLE =
  "flex items-center justify-center px-[18px] py-[8px] text-[11px] leading-[13px] font-[590]";
...
/**
 * Figma `Input Mode Toggle`(node `342-833`, variant `342:832`/`342:834`, fileKey
 * `ltyPrCk8UT8DsB3tFuw7Sr`) — `/solve/pencilcanvas` INPUT 단계 전용 "사진으로 문제 인식" /
 * "필기로 문제 인식" 세그먼트 토글. NavTabBar 바로 아래 화면 상단 중앙에 배치된다(배치는 이
 ...
 * `navigate("/camera")`를 실행한다. 이 컴포넌트 자체는 라우팅 로직을 갖지 않는 순수 컴포넌트다.
 *
...
export function InputModeToggle({ mode, onSelectMode }: InputModeToggleProps) {
  return (
    <div
      className={
        "bg-fill-tint-green flex h-[36px] items-center gap-[8px] rounded-[6px] p-[6px] " +
