...
export type PostAuthDestination = "/grade-setup" | "/solve";

/**
 * 로그인/인증 후 이동할 목적지를 user_metadata.grade 존재 여부로 판단하는 단일 기준 함수.
 * 학년이 설정되어 있지 않으면 /grade-setup, 설정되어 있으면 /solve로 이동시킨다.
...
export function getPostAuthDestination(user: User | null): PostAuthDestination {
  ...
  return typeof grade === "string" && grade.length > 0 ? "/solve" : "/grade-setup";
}
