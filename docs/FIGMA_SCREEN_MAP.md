...
- **2026-09-06 design-agent 정밀 실측(문제 인식 스토리보드 `267:778`)**: `3-1 · Solve/Pencilcanvas`는 실제로 `_1`(`127:445`, 풀이 전, variant `Stage=Notyet`=`267:682`)과 `_2`(`267:607`, 풀이 후, variant `Stage=Work`=`249:69`) 두 sub-state 프레임으로 존재하며, 신규 라우트를 만들지 않고 `hasWorkInput` 신호로 같은 화면 내에서 분기한다. ActionBar는 4단계(INPUT/WORK-풀이전/WORK-풀이후/RESULT) 상태표를 가지며, RESULT 단계에서는 첫 번째 세그먼트가 "새 문제 풀기"로 라벨이 바뀌고 활성/강조된다(variant `Stage=NewProblem`=`267:454`). 이번 조사에서 로딩 마크(`190:866`, 배경/카드 없는 순수 마크), INPUT 빈 상태 힌트(`260:454`), WORK 빈 상태 힌트(`242:528`, `_1`에만 존재), 캔버스 상단 "인식됨" 칩(`Solve/Recognized Chip`, 마스터 `250:56`/인스턴스 `256:373`)이 신규로 확인됐다. 해시태그 pill(`#개념설명`(고정) + 관련개념 태그(가변) + `#비슷한 문제`(고정))과 ResultPanel `Style=V1.0 old`(`174:638`)/`V2_Default`(`253:53`)/`V2_Extend`(`174:640`, 최하단에 "관련개념" ResultCard 추가)/`Close`(`174:743`) variant도 이번에 확인됨. 상세는 `docs/COMPONENT_MAP.md` §1/§2, `docs/PROJECT_STATUS.md` §3.21 참고.

- **2026-09 "사진 업로드" 추가(INPUT-4)**: `3-0-0 · Solve/Default(Camera)`(`260-423`)의 `Input Mode Toggle`(node `342-833`)이 2분할에서 3분할("사진으로 문제 인식"/"사진 업로드"/"필기로 문제 인식", 세그먼트 사이 세로 구분선 2개)로 갱신됐다. 업로드는 별도 프레임/라우트 없이 같은 화면에서 처리한다. Figma에는 `Select=Camera`/`Select=Pencil` 두 variant만 있고 "사진 업로드 선택" variant·업로드 오류 화면·안내 문구는 없다(선택 시각은 기존 선택 pill 스타일 파생, 오너 확정 대기). 세 탭의 Figma 레이어 이름은 `Tab/Camera`/`Tab/Pencil`을 재사용해 중간 탭의 이름이 실제 역할과 다르다.
